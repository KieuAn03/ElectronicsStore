...
def add_to_cart(request , **kwargs):
    ...
    if(str(product.product_type) == 'Phone'):
        ...
        if(kwargs.get('ram')):
            ...
            print(customs)
            custom = PhoneOptionHard.objects.first()
            ...
            if(kwargs.get('color')):  
                ...
                color = PhoneOptionColor.objects.first()    
                cart_item = cart_item_phone.objects.create(cart=cart , product = phone , quantity = 1 ,price_add= custom,color = PhoneColor.objects.get(name = kwargs.get('color'))  )
                cart_item.save()    
                print("SAVVVVVVEEEE")
            else:
                color = PhoneOptionColor.objects.first()    
            ...
            cart_item = cart_item_phone.objects.create(cart = cart , phone = product ,quantity = 1, price_add= custom,color = PhoneColor.objects.get(name = kwargs.get('color')))
            cart_item.save()
        else:
            custom = PhoneOptionHard.objects.get(ram = phone.ram , storage = phone.storage)
